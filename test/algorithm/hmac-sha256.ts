...
describe('hmac-sha256', () => {
    ...
    describe('internal tests', () => {
        ...
        it('signs a payload correctly', async () => {
            const hmac = createSigner('key', 'hmac-sha256');
            expect(hmac.alg).to.equal('hmac-sha256');
            expect(await hmac.sign(data)).to.deep.equal(sig);
        });
        it('verifies a payload correctly', async () => {
            const hmac = createVerifier('key', 'hmac-sha256');
    ...
    describe('specification examples', () => {
        ...
        it('generates an expected hmac', async () => {
            const hmac = createSigner(testSharedSecret, 'hmac-sha256');
            const sig = await hmac.sign(signatureInput);
            expect(hmac.alg).to.equal('hmac-sha256');
            expect(sig).to.deep.equal(expectedSig);
        });
        it('verifies a provided signature', async () => {
            const hmac = createVerifier(testSharedSecret, 'hmac-sha256');
